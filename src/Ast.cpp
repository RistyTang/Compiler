...
void BinaryExpr::genCode() 
{
    ...
    if (op == AND) 
    {
        ...
        //如果expr1不是bool类型——转换
        if(expr1->getSymbolEntry()->getType()->toStr()=="i32"||expr1->getSymbolEntry()->getType()->toStr()=="i32()")
        {
            BasicBlock* condbb=expr1->builder->getInsertBB();
            Function* tempfunc = expr1->builder->getInsertBB()->getParent();
            BasicBlock* tempbb = new BasicBlock(tempfunc);
            BasicBlock* falseBB = new BasicBlock(tempfunc);
            Operand *tempdst = new Operand(new TemporarySymbolEntry(TypeSystem::boolType, SymbolTable::getLabel()));
            //在不相等时跳转，因此为NE
            new CmpInstruction(CmpInstruction::NE,//opcode
                                tempdst,//dst
                                expr1->getOperand(),//src1
                                nullptr,//src2
                                bb);//int转bool
            new CondBrInstruction(trueBB,nullptr,tempdst,bb);
            expr1->trueList().push_back(new CondBrInstruction(trueBB, tempbb, tempdst, condbb));
        ...
        bb=builder->getInsertBB();
        //如果expr2不是bool类型——转换
        if(expr2->getSymbolEntry()->getType()->toStr()=="i32"||expr2->getSymbolEntry()->getType()->toStr()=="i32()")
        {
            BasicBlock* condbb=expr2->builder->getInsertBB();
            Function* tempfunc = expr2->builder->getInsertBB()->getParent();
            BasicBlock* tempbb = new BasicBlock(tempfunc);
            BasicBlock* falseBB = new BasicBlock(tempfunc);
            Operand *tempdst = new Operand(new TemporarySymbolEntry(TypeSystem::boolType, SymbolTable::getLabel()));
            //在不相等时跳转，因此为NE
            new CmpInstruction(CmpInstruction::NE,//opcode
                                tempdst,//dst
                                expr2->getOperand(),//src1
                                new Operand(new ConstantSymbolEntry(TypeSystem::intType, 0)),//src2
                                bb);//int转bool
            new CondBrInstruction(trueBB,nullptr,tempdst,bb);
            expr2->trueList().push_back(new CondBrInstruction(trueBB, tempbb, tempdst, condbb));
