    ...
    if (op >= BinaryExpr::AND && op <= BinaryExpr::NOTEQUAL) 
    {
        ...
        if (op == BinaryExpr::AND || op == BinaryExpr::OR) 
        {
            if (expr1->getType()->isInt() && expr1->getType()->getSize() == 32) 
            {
                ...
                this->expr1 = temp;
                /*
                this->expr1->SetType(TypeSystem::boolType);
                Operand* newdst=new Operand(new TemporarySymbolEntry(type, SymbolTable::getLabel()));
                this->expr1->SetDst(newdst);
                this->expr1->SetKind(2);
                this->expr1->SetSymbolEntry(nullptr);
                */
            }
