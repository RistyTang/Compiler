...
void BinaryExpr::genCode() 
{
    ...
    else if (op >= LESS && op <= NOTEQUAL) 
    {
        ...
        //如果有一方是bool类型，使用一个扩展语句，否则102报错i32 i1
...
void FunctionDef::genCode() 
{
    ...
    for (auto block = func->begin(); block != func->end(); block++) 
    {
        ...
        else if (last->isUncond())  //无条件跳转指令可获取跳转的目标块
        {
            ...
            (*block)->addSucc(dst);
            dst->addPred(*block);
            //027报错，B7缺少一条return语句
            //跳转为空
            if (dst->empty()) 
            {
                if (((FunctionType*)(se->getType()))->getRetType() == TypeSystem::intType)
                {    
                    Operand* temp=new Operand(new ConstantSymbolEntry( TypeSystem::intType, 0));
                    new RetInstruction(temp, dst);
                }
            }
